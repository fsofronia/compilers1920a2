...
rexp2 = re.compile(r'(<script(.+?)</script>)|(<style(.+?)</style>)') #apalifh srcipt kai style
rexp3 = re.compile(r'(<a)(.+?)/a>') # ebresh <a> kai </a> 
rexp4 = re.compile(r'href="(.+?)">(.+?)<') # href
...
rexp7 = re.compile(r'\s+') #evresh kenwn kai new lines
with open('testpage.txt','r') as fp: #anagnwsh testpage.html
	...
	for m2 in rexp3.finditer(newtext1): #gia kathe <a> </a>
		for m3 in rexp4.finditer(m2.group(2)): #briskw to href pou perilambanetai sto <a> </a>
			print(m3.group(1),m3.group(2),file=f) #ektypwnw (sto arxeio output.txt) me thn xrhsh twn groups to link kai titlo
	newtext2 = rexp5.sub('',newtext1) # antikatastash oswn briskei h rexp5 me keno(apalifh twn sxoliwn)
	newtext3 = rexp6.sub(repl,newtext2) #allagh twn oswn briskei h rexp6 ($amp, %gt..)me bash thn sunartish repl 
